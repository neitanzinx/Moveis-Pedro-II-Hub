...
export default function VendaModal({ isOpen, onClose, onSave, venda, clientes, produtos, isLoading, userLoja, proximoNumero }) {
    ...
    try {
      ...
      for (const pag of formData.pagamentos) {
        ...
        if (config) {
          ...
          porcentagemMedia += config.porcentagem;
        }
      }

      if (formData.pagamentos.length > 0) {
        porcentagemMedia = porcentagemMedia / formData.pagamentos.length;
      }

      setFormData(prev => ({
        ...prev,
        comissao_calculada: comissaoTotal,
        porcentagem_comissao: porcentagemMedia
      }));
    } catch (error) {
  ...
  const adicionarProdutoSelecionado = () => {
    if (!selectedProduto || quantidade <= 0 || quantidade > 5) {
      if (quantidade > 5) {
        alert("A quantidade máxima por item é 5 unidades");
      }
      return;
    }
  ...
  const adicionarPagamento = () => {
    ...
    if (novoTotal > formData.valor_total) {
      alert(`O total de pagamentos (R$ ${novoTotal.toFixed(2)}) não pode exceder o valor total da venda (R$ ${formData.valor_total.toFixed(2)})`);
      return;
    }
    ...
    if (!venda) {
      ...
      if (formData.prazo_entrega !== "Retirado na loja") {
        ...
        await base44.entities.Entrega.create({
          venda_id: formData.numero_pedido, // será atualizado após salvar
